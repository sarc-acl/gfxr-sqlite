...
static void CreatePhysicalDevicePropertiesTables(sqlite3* db)
{
    ...
    // VulkanSqliteConsumerExt::Process_vkGetPhysicalDeviceProperties[2] - one row per physical device
    // whose properties were actually queried. `physicalDeviceId` is UNIQUE (paired with
    // `INSERT OR IGNORE`) because apps commonly call vkGetPhysicalDeviceProperties[2] many times per
    // device; the properties are immutable, so only the first call's row is kept.
    ExecSQL(
        db,
        "CREATE TABLE physicalDeviceLimits("
        "   physicalDeviceId INT UNIQUE NOT NULL,"
        ...
        "   maxComputeWorkGroupCountX INT,"
        "   maxComputeWorkGroupCountY INT,"
        "   maxComputeWorkGroupCountZ INT,"
        ...
        "   maxViewports INT,"
        "   maxViewportDimensionsWidth INT,"
        "   maxViewportDimensionsHeight INT,"
        "   viewportBoundsRangeMin REAL,"
        "   viewportBoundsRangeMax REAL,"
        ...
        "   framebufferDepthSampleCounts INT,"
        "   framebufferStencilSampleCounts INT,"
        "   framebufferNoAttachmentsSampleCounts INT,"
        "   maxColorAttachments INT,"
        "   sampledImageColorSampleCounts INT,"
        "   sampledImageIntegerSampleCounts INT,"
        "   sampledImageDepthSampleCounts INT,"
        "   sampledImageStencilSampleCounts INT,"
        "   storageImageSampleCounts INT,"
        "   maxSampleMaskWords INT,"
        "   timestampComputeAndGraphics INT,"
        ...
        "   pointSizeRangeMin REAL,"
        "   pointSizeRangeMax REAL,"
        "   lineWidthRangeMin REAL,"
        "   lineWidthRangeMax REAL,"
        "   pointSizeGranularity REAL,"
        "   lineWidthGranularity REAL,"
        "   strictLines INT,"
        "   standardSampleLocations INT,"
        ...
        "   FOREIGN KEY(timestampComputeAndGraphics) REFERENCES VkBool32(value),"
        "   FOREIGN KEY(strictLines) REFERENCES VkBool32(value),"
        "   FOREIGN KEY(standardSampleLocations) REFERENCES VkBool32(value)) STRICT;"
    );
    ExecSQL(
        db,
        "CREATE TABLE physicalDeviceSparseProperties("
        "   physicalDeviceId INT UNIQUE NOT NULL,"
        "   residencyStandard2DBlockShape INT,"
        "   residencyStandard2DMultisampleBlockShape INT,"
        "   residencyStandard3DBlockShape INT,"
        "   residencyAlignedMipSize INT,"
        "   residencyNonResidentStrict INT,"
        "   FOREIGN KEY(physicalDeviceId) REFERENCES physicalDevices(id),"
        "   FOREIGN KEY(residencyStandard2DBlockShape) REFERENCES VkBool32(value),"
        "   FOREIGN KEY(residencyStandard2DMultisampleBlockShape) REFERENCES VkBool32(value),"
        "   FOREIGN KEY(residencyStandard3DBlockShape) REFERENCES VkBool32(value),"
        "   FOREIGN KEY(residencyAlignedMipSize) REFERENCES VkBool32(value),"
        "   FOREIGN KEY(residencyNonResidentStrict) REFERENCES VkBool32(value)) STRICT;"
    );
}
...
static void CreateAccelerationStructureTables(sqlite3* db)
{
    ...
    ExecSQL(
        ...
        "   deviceId INT,"
        "   frameId INT,"
        "   deferredOperationId INT,"
        "   commandBufferRecordingId INT,"
        "   apiEventId INT NOT NULL,"
        "   FOREIGN KEY(deviceId) REFERENCES devices(id),"
        "   FOREIGN KEY(frameId) REFERENCES frames(id) ON DELETE SET NULL,"
        "   FOREIGN KEY(deferredOperationId) REFERENCES deferredOperations(id),"
        "   FOREIGN KEY(commandBufferRecordingId) REFERENCES commandBufferRecordings(id),"
        "   FOREIGN KEY(apiEventId) REFERENCES apiEvents(id)) STRICT;"
    );
